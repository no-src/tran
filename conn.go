...
func (conn *Conn) StartAuthCheck() {
	if !conn.startAuthCheck.Load() {
		conn.startAuthCheck.Store(true)
		conn.authCheck()
	}
}
...
func (conn *Conn) StopAuthCheck() {
	conn.startAuthCheck.Store(false)
}
